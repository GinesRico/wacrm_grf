...
[![Postgres](https://img.shields.io/badge/Postgres-Postgres%20%2B%20Auth-3ecf8e?logo=postgres)](https://postgres.com)
...
cd wacrm
npm install
cp .env.local.example .env.local   # fill in Postgres + Meta creds
npm run dev
```
...
| **Env vars + logs in hPanel** | Set `DATABASE_URL`, `BETTER_AUTH_SECRET`, `WHATSAPP_*`, `ALARIK_*`, `SOKETI_*`, and `ENCRYPTION_KEY` from the panel — no `.env` on the server. Live application logs in the same UI. |
...
| **Cheaper than a VPS** | Plans start at a few dollars a month — order-of-magnitude less than a comparable managed Node.js host, and you don't pay extra for the database (that's Postgres). |
...
2. In **hPanel → Websites → Create**, pick **Node.js** and connect
   your fork.
3. Paste your Postgres + Meta env vars into hPanel.
...
Full self-host documentation — Drizzle migrations, WhatsApp Business
...
- [Postgres setup](https://wacrm.tech/docs/postgres-setup)
...
- **Data** — Postgres (Postgres + Auth + Storage + RLS).
