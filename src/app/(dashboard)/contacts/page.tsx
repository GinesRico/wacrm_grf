...
export default function ContactsPage() {
  ...
  const fetchTags = useCallback(async () => {
    ...
    const data = (payload.tags as Tag[] | undefined) ?? [];
    if (res.ok) {
  ...
  async function openEditForm(contact: Contact) {
    ...
    const payload = await res.json().catch(() => ({}));
    const tagIds = Array.isArray(payload.contact_tag_ids)
      ? (payload.contact_tag_ids as string[])
      : [];
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/contacts?id=${deleteTarget.id}`, {
      method: 'DELETE',
    });
