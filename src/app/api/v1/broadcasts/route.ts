...
export async function POST(request: Request) {
  try {
    ...
    const auditUserId = await resolveAuditUserId(null, ctx.accountId);

    const plan = await createBroadcast(null, ctx.accountId, auditUserId, {
    ...
    after(() => deliverBroadcast(null, plan));
