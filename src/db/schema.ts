...
export const platformAccountInvites = pgTable(
  ...
  (table) => [
    index("idx_platform_account_invites_pending").on(table.expiresAt),
  ],
);
...
export const contactTags = pgTable(
  ...
  (table) => [
    uniqueIndex("contact_tags_contact_tag_key").on(table.contactId, table.tagId),
...
export const customFields = pgTable(
  ...
  (table) => [
    uniqueIndex("custom_fields_account_name_key").on(table.accountId, table.fieldName),
...
export const departmentMembers = pgTable(
  ...
  (table) => [
    ...
    index("idx_department_members_account_user").on(table.accountId, table.userId),
  ],
);
...
export const conversations = pgTable(
  "conversations",
  {
    ...
    aiAutoreplyDisabled: boolean("ai_autoreply_disabled").notNull().default(false),
  ...
  (table) => [
    ...
    index("idx_conversations_last_message").on(table.accountId, table.lastMessageAt),
  ],
);
...
export const automationSteps = pgTable(
  ...
  (table) => [
    ...
    check("automation_steps_branch_check", sql`${table.branch} in ('yes', 'no')`),
  ],
);
...
export const automationLogs = pgTable(
  ...
  (table) => [
    ...
    check("automation_logs_status_check", sql`${table.status} in ('success', 'partial', 'failed')`),
  ],
);
...
export const automationPendingExecutions = pgTable(
  ...
  (table) => [
    ...
    check("automation_pending_executions_branch_check", sql`${table.branch} in ('yes', 'no')`),
    check("automation_pending_executions_status_check", sql`${table.status} in ('pending', 'running', 'done', 'failed')`),
  ],
);
...
export const flows = pgTable(
  ...
  (table) => [
    ...
    check("flows_status_check", sql`${table.status} in ('draft', 'active', 'archived')`),
    check("flows_trigger_type_check", sql`${table.triggerType} in ('keyword', 'first_inbound_message', 'manual')`),
  ],
);
...
export const flowNodes = pgTable(
  ...
  (table) => [
    uniqueIndex("flow_nodes_flow_id_node_key_key").on(table.flowId, table.nodeKey),
    index("idx_flow_nodes_flow").on(table.flowId),
    check("flow_nodes_node_type_check", sql`${table.nodeType} in ('start', 'send_buttons', 'send_list', 'send_message', 'send_media', 'collect_input', 'condition', 'set_tag', 'handoff', 'http_fetch', 'end')`),
  ],
);
...
export const flowRuns = pgTable(
  ...
  (table) => [
    ...
    check("flow_runs_status_check", sql`${table.status} in ('active', 'completed', 'handed_off', 'timed_out', 'paused_by_agent', 'failed')`),
  ],
);
...
export const flowRunEvents = pgTable(
  ...
  (table) => [
    ...
    check("flow_run_events_event_type_check", sql`${table.eventType} in ('started', 'node_entered', 'message_sent', 'reply_received', 'fallback_fired', 'handoff', 'timeout', 'error', 'completed')`),
  ],
);
...
export const integrationConnections = pgTable(
  ...
  (table) => [
    ...
    check("integration_connections_status_check", sql`${table.status} in ('not_configured', 'active', 'disabled', 'error')`),
  ],
);
...
export const paymentLinks = pgTable(
  ...
  (table) => [
    ...
    index("idx_payment_links_account_created").on(table.accountId, table.createdAt),
    ...
    check("payment_links_status_check", sql`${table.status} in ('pending', 'paid', 'failed', 'expired', 'cancelled')`),
  ],
);
...
export const appointmentRecords = pgTable(
  ...
  (table) => [
    ...
    index("idx_appointment_records_account_start").on(table.accountId, table.startTime),
...
export const appointmentAvailabilityMessages = pgTable(
  ...
  (table) => [
    ...
    check("appointment_availability_send_mode_check", sql`${table.sendMode} in ('booking_link', 'interactive_list', 'cta_url')`),
  ],
);
...
export const aiUsageLog = pgTable(
  ...
  (table) => [
    index("idx_ai_usage_log_account_created").on(table.accountId, table.createdAt),
    check("ai_usage_log_mode_check", sql`${table.mode} in ('auto_reply', 'draft')`),
    check("ai_usage_log_provider_check", sql`${table.provider} in ('openai', 'anthropic')`),
  ],
);
...
export const aiConfigs = pgTable(
  ...
  (table) => [
    check("ai_configs_provider_check", sql`${table.provider} in ('openai', 'anthropic')`),
    check("ai_configs_auto_reply_max_check", sql`${table.autoReplyMaxPerConversation} between 1 and 20`),
  ],
);
...
export const aiKnowledgeDocuments = pgTable(
  ...
  (table) => [index("ai_knowledge_documents_account_id_idx").on(table.accountId)],
);
