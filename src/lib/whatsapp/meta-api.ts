...
export async function sendTemplateMessage(
  ...
  if (template) {
    const components = buildSendComponents(template, {
      ...
      flowActionData: messageParams?.flowActionData,
    });
...
export async function sendInteractiveFlow(
  ...
  validateInteractiveBody(bodyText);
  validateInteractiveHeaderFooter(headerText, footerText);
  if (!flowId) throw new Error('Interactive Flow requires flowId.');
  if (!flowCta) throw new Error('Interactive Flow requires flowCta.');
  if (flowCta.length > INTERACTIVE_LIMITS.buttonTitleMaxLength) {
    throw new Error(
      `Interactive Flow CTA "${flowCta}" exceeds ${INTERACTIVE_LIMITS.buttonTitleMaxLength} chars.`
    );
  }
  ...
  if (flowAction) parameters.flow_action = flowAction;
  if (screen || data) {
...
/**
 * Send a free-form interactive CTA URL button message. This is distinct
 * from reply buttons: WhatsApp opens the URL instead of sending a reply
 * webhook for routing.
 */
export async function sendInteractiveCtaUrl(
  ...
  validateInteractiveHeaderFooter(headerText, footerText);
  if (!buttonLabel)
    throw new Error('Interactive CTA URL requires a button label.');
  if (buttonLabel.length > INTERACTIVE_LIMITS.buttonTitleMaxLength) {
    throw new Error(
      `Interactive CTA URL button label "${buttonLabel}" exceeds ${INTERACTIVE_LIMITS.buttonTitleMaxLength} chars.`
    );
  }
