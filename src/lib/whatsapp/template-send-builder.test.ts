...
describe('buildSendComponents — buttons', () => {
  ...
  it('emits a FLOW button with root flow_action_data', () => {
    const components = buildSendComponents(
      row({
        buttons: [{ type: 'FLOW', text: 'Reservar', flow_id: '12345' }],
      }),
      {
        buttonParams: { 0: 'signed-token' },
        flowActionData: {
          0: {
            service: [{ id: 'neumaticos', title: 'Neumaticos' }],
            date: [{ id: '2026-08-20', title: 'jueves 20 agosto' }],
            time: [{ id: 'pending_date', title: 'Selecciona una fecha' }],
          },
        },
      }
    );
  ...
  it('uses the real FLOW button index from the template', () => {
    const components = buildSendComponents(
      row({
        buttons: [
          { type: 'QUICK_REPLY', text: 'Info' },
          { type: 'FLOW', text: 'Reservar', flow_id: '12345' },
        ],
      }),
      { buttonParams: { 1: 'signed-token' } }
    );
