...
describe('sendInteractiveFlow', () => {
  ...
  it('sends the Meta Flow payload with flow_action_payload.data', async () => {
    let captured: { body: unknown } | null = null;
    vi.stubGlobal(
      'fetch',
      vi.fn(async (_url: string, init: RequestInit) => {
        captured = { body: JSON.parse(String(init.body)) };
        return new Response(
          JSON.stringify({ messages: [{ id: 'wamid.FLOW' }] }),
          { status: 200 }
        );
      })
    );
    ...
    expect(result).toEqual({ messageId: 'wamid.FLOW' });
    expect(captured!.body).toMatchObject({
      messaging_product: 'whatsapp',
      recipient_type: 'individual',
      to: '1234567890',
      ...
      interactive: {
        type: 'flow',
        header: { type: 'text', text: 'Reservar cita' },
        body: { text: 'Body text' },
        footer: { text: 'Autorecambios Vera' },
        action: {
          name: 'flow',
          parameters: {
