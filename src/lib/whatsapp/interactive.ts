...
export function validateInteractivePayload(
  ...
  if (p.kind === 'flow') {
    const flow = p as Partial<InteractiveFlowPayload>;
    if (typeof flow.flow_id !== 'string' || flow.flow_id.trim() === '') {
      return fail('The WhatsApp Flow needs a flow_id.');
    }
    if (typeof flow.flow_cta !== 'string' || flow.flow_cta.trim() === '') {
      return fail('The WhatsApp Flow needs a button label.');
    }
    if (flow.flow_cta.length > INTERACTIVE_LIMITS.buttonTitleMaxLength) {
      return fail(
        `Flow button label exceeds the ${INTERACTIVE_LIMITS.buttonTitleMaxLength}-character limit.`
      );
    }
    if (
      flow.flow_action &&
      ...
      return fail('Flow action must be navigate or data_exchange.');
    }
    if (flow.screen !== undefined && typeof flow.screen !== 'string') {
      return fail('Flow screen must be a string.');
    }
    if (
      flow.data !== undefined &&
      (!flow.data || typeof flow.data !== 'object' || Array.isArray(flow.data))
    ) {
      return fail('Flow data must be an object.');
    }
    return ok();
  }

  return fail(
    'Interactive message must be reply buttons, a list, a URL button, or a WhatsApp Flow.'
  );
}
